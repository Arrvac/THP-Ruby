...
puts "La longeur de ce tab est de #{TAB.length}"

def check_little_word
	little = 0
	0.upto TAB.length - 1 do |a|
		if TAB[a].length < little || little == 0
			little = TAB[a].length.to_i
		end
	end
	puts "La longeur du plus petit mot de ce tab est de #{little} char"
end
...
def check_5_letter_word
	...
	puts "Il y a #{word} mot de 5 char"
end

def check_maj
	word = 0
	0.upto TAB.length - 1 do |a|
		if TAB[a][1] >= 'A' && TAB[a][1] <= 'Z'
			word += 1
		end
	end
	puts "Il y a #{word} mot avec des premiere lettre maj"
end
