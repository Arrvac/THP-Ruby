...
def check_5_letter_word
	puts "Il y a #{ TAB.count { |num| num.length == 6 } } handle de 5 char"
end

def check_maj
	puts "Il y a #{ TAB.count { |num| num[1] == num[1].upcase }} handle qui commencent par une maj"
end
...
def tab_sort_length
	pp TAB.sort { |a, b| a.length <=> b.length }
end

def index_epenser
	puts "epenser est a l'index #{ TAB.index("@epenser") } "
end
